...
const getBarHeight = (value, maxValue, maxHeightPixels = 85) => {
  // Reduced from 100 to 85
  if (value === 0) return 4; // Minimum height for visibility
  return Math.max(
    4,
    Math.min(maxHeightPixels, (value / maxValue) * maxHeightPixels),
  );
};
...
export default function Analytics() {
  ...
  async function fetchData(period) {
    ...
    try {
      ...
      const catWithPercent = catList.map((c) => ({
        ...c,
        percent:
          totalExp > 0 ? ((c.amount / totalExp) * 100).toFixed(0) + "%" : "0%",
        width:
          totalExp > 0 ? ((c.amount / totalExp) * 100).toFixed(0) + "%" : "0%",
      }));
    ...
    if (period === "W") {
      ...
      txData.forEach((t) => {
        ...
        if (groups[dateStr]) {
          ...
          groups[dateStr].net =
            groups[dateStr].income - groups[dateStr].expenses;
        }
      });
    } else if (period === "M") {
        ...
        for (const range of weekRanges) {
          if (txDate >= range.startDate && txDate <= range.endDate) {
            if (groups[range.key]) {
              ...
              groups[range.key].net =
                groups[range.key].income - groups[range.key].expenses;
            }
            break;
          }
        }
      ...
      for (let i = 6; i >= 1; i--) {
        ...
        const yearSuffix =
          year !== today.getFullYear() ? ` ${year.toString().slice(-2)}` : "";
  ...
  const chartItems =
    chartData.length > 0
      ? chartData
      : [{ id: "empty", label: "-", income: 0, expenses: 0, net: 0 }];
  ...
  const statsCards = [
    ...
    {
      label: "Saved",
      value: `$${Math.max(stats.saved, 0).toLocaleString()}`,
      change: "this period",
      trend: "up",
    },
    {
      ...
      change: stats.savingsRate >= 20 ? "On track" : "Below target",
        ...
        <View style={styles.periodTabs}>
          {periods.map((period) => (
            <TouchableOpacity
              ...
              style={[
                styles.periodTab,
                period === selectedPeriod && styles.periodTabActive,
              ]}
              activeOpacity={0.7}
              ...
              {tooltip.visible && (
                <View style={styles.globalTooltip}>
                  <Text
                    style={
                      tooltip.type === "income"
                        ? styles.globalTooltipIncome
                        : styles.globalTooltipExpense
                    }
                  >
                    {tooltip.type === "income" ? "Income: " : "Expenses: "}
                    {tooltip.value}
                  </Text>
                </View>
              )}
              ...
              >
                <View style={styles.chartBars}>
                  {chartItems.map((item, index) => {
                    const incomeHeight =
                      item.income > 0
                        ? Math.max(8, (item.income / maxValue) * 100)
                        : 4;
                    const expenseHeight =
                      item.expenses > 0
                        ? Math.max(8, (item.expenses / maxValue) * 100)
                        : 4;
          ...
          {statsCards.map((item) => (
            <View key={item.label} style={styles.statCard}>
              ...
              <Text
                style={
                  item.trend === "up"
                    ? [styles.statChange, styles.statChangeUp]
                    : [styles.statChange, styles.statChangeDown]
                }
              >
                {item.change}
              </Text>
